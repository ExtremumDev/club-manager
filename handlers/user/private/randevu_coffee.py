...
async def send_coffe_info(c: types.CallbackQuery, db_session, *args):
    ...
    if user.randevu_notifications:
        text = "У вас включено получение предложений о встрече. Чтобы выключить👇"
    else:
        text = "У вас сейчас выключено получение предложений о встрече. Чтобы включить👇"
