...
def get_randevu_settings_text(randevu_notifications: bool):
    if randevu_notifications:
        return "На этой неделе вы участвуете в встречах рандеву-клуба. Если хотите отказаться от предложений на этой неделе👇"
    else:
        return "На этой неделе вы не получите предложение о встрече. Если передумаете👇"
...
async def switch_mode(c: types.CallbackQuery, db_session, *args):
    ...
    if new_mode: # new_mode == 1 (turn on)
        text = "Теперь вы получите предложения о встрече на этой неделе"
    else: # turn off
        text = "Вы не получите предложения о встрече на этой неделе, пока не включите обратно"
